...
function getVernietigingsStatusClasses(
  status: Connector["vernietigingsStatus"]
) {
  switch (status) {
    case "NIET_GESTART":
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <PageHeader
        ...
        actions={[
          {
            label: "vernietiging uitvoeren",
            variant: "primary",
            icon: <ArrowRight className="h-3.5 w-3.5" />,
            onClick: () =>
              navigate(
                `/taak/${taakId}/taakuitvoering/${id}/resultaat`
              ),
          },
        ]}
      />

      <WorkflowBar activeStep="UITVOERING" />
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-gray-900">
              Wat gebeurt er bij vernietiging uitvoeren?
            </h3>
            <p className="mt-1 max-w-3xl text-sm leading-6 text-gray-600">
              De vastgestelde vernietigingslijst wordt per stekker uitgevoerd.
              Per bron wordt de voortgang van de vernietiging bijgehouden totdat
              alle geselecteerde objecten zijn verwerkt.
            </p>
          </div>
        </div>
      </div>
      ...
      <section className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        <div className="border-b border-gray-200 px-6 py-5">
          ...
          <p className="mt-1 text-sm text-gray-500">
            Beschikbare bronnen voor vernietiging
          </p>
        </div>
        ...
        <table className="min-w-full divide-y divide-gray-200 text-sm">
          <thead className="bg-gray-50">
            <tr className="text-left text-xs font-semibold uppercase tracking-[0.08em] text-gray-400">
              ...
              <th className="px-6 py-3">Vernietigings status</th>
          ...
          <tbody className="divide-y divide-gray-200 bg-white">
            {connectors.map((connector) => (
              <tr key={connector.id} className="text-gray-700">
                ...
                <td className="px-6 py-4">
                  {renderStekkerStatus(connector.stekkerStatus)}
                </td>
                <td className="px-6 py-4">
                  <div className="min-w-[220px]">
                    <div className="flex items-center justify-between gap-3">
                      <span
                        className={`inline-flex items-center rounded-md border px-2 py-0.5 text-xs font-medium ${getVernietigingsStatusClasses(
